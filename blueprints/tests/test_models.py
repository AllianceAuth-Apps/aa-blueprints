...
class TestOwner_UpdateBlueprintsESI(TestCaseWithClearCache):
    ...
    @pook.on
    def test_should_update_existing_blueprints_when_owner_changed(self):
        # given
        owner = OwnerCharacterFactory()
        ...
        self.assertEqual(bp_2.runs, runs)
        self.assertEqual(bp_2.time_efficiency, time_efficiency)
        self.assertEqual(bp_2.owner, owner)
...
class TestOwner_UpdateIndustryJobsEsi(TestCaseWithClearCache):
    ...
    def test_should_reset_existing_blueprint_relation(self):
        ...
        stale_job = IndustryJobFactory(blueprint=bp, location=location)
        job_id = 42
        end_date = now() + dt.timedelta(hours=3)
        start_date = now()
        duration = int((end_date - start_date).total_seconds())
